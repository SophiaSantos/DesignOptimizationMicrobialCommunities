...
SBML_FILE = (os.path.join(basePath, 'iSS352.xml'))
...
essential = ['R_EX_C14818__dra', 'R_EX_C14819__dra']

minimal_medium = ['R_EX_C00014__dra', 'R_EX_C00009__dra', 'R_EX_C00087__dra']
...
for i in starvation:
    constraints.update({i: (-0.267, 0)})

#for i in KO:
#    constraints.update({i: (0, 0)})

for i in acetate:
    constraints.update({i: (-20, -20)})
...
for j in carbon_source:
    ...
    if pfba.solverStatus == 1:
        ...
        fva = FVA(newModel, obj_percentage=0.1,reactions=['R_EX_C00760__dra','R_EX_C00288__dra', 'R_EX_C00993__dra', 'R_EX_C00033__dra'], constraints=constraints)
        ...
        #essentialRxn = simulProb.find_essential_drains()
